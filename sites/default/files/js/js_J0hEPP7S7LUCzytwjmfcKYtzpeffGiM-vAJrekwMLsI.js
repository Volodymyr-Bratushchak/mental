...
;
(function ($) {
  $('#generator_block button').off().on('click',(function () {
    event.preventDefault();
    $('#news_section').remove(); // Delete existing
    var url = $(this).attr("href");
    var items = [];
    $.getJSON(url, function (data) {
      $.each(data, function (key, fact) {
        items.push( "<div id='fact#" + fact.name + "'><a href='/admin/structure/fact/2" + fact.nid + "'>" +
          node.title + "<img src='" + node.image + "'></a></div>" );
      });
      $( "<div/>", {
        "id": "fact",
        html: items.join( "" )
      }).appendTo( "#generator_block" );
    });
  }));
})(jQuery);
...
//   // load() functions
//   var loadUrl = "http://mental.loc/node/25";
//
//   $("#loadbasic").off().on('click',(function (){
